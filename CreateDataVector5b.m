%% Script info
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% File name: "CreateDataVector5b.m"                            %
% Purpose: Reads the hex_data vector that was created          %
% by LSTDataRead.m and creates a photon table of that          %
% measurement. Only reads the time_patch == 5b data.           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%

function [Final_Dataset] = CreateDataVector5b(Raw_Data, Data_Channel_Num, Range)
%% Check input
...
switch Data_Channel_Num
    case 1
        ...
        TAG_Bits = Data_Readings(:,1:15); % reads TAG bits data and converts it to decimal
        Sweep_Counter = bin2dec(Data_Readings(:,16:31)); % reads sweep counter data and converts it to decimal
        Time_of_Arrival_Before_Sweep_Correction = bin2dec(Data_Readings(:,32:59)); % reads timestamp data and converts it to decimal
    case 2

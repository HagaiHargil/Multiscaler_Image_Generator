%% Script info
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% File name: "CreateDataVector1.m"                             %
% Purpose: Reads the hex_data vector that was created          %
% by LSTDataRead.m and creates a photon table of that          %
% measurement. Only reads the time_patch == 1 data.            %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%

function [Final_Dataset] = CreateDataVector1(Raw_Data, Data_Channel_Num, Range)
%% Check input
...
switch Data_Channel_Num
    case 1
        helpstr = repmat('001', size(Raw_Data, 1), 1) == Raw_Data(:,30:32); % helpmat locates the rows in which PMT data was read
        Data_Readings = Raw_Data((sum(helpstr, 2) == 3),1:28); % creates a vector only containing PMT data
        Time_of_Arrival = bin2dec(Data_Readings(:,1:28)); % reads timestamp data and converts it to decimal
    case 2
        helpstr = repmat('010', size(Raw_Data, 1), 1) == Raw_Data(:,30:32); % helpmat locates the rows in which PMT data was read
        Data_Readings = Raw_Data((sum(helpstr, 2) == 3),1:28); % creates a vector only containing PMT data
        Time_of_Arrival = bin2dec(Data_Readings(:,1:28));
    case 6
        helpstr = repmat('110', size(Raw_Data, 1), 1) == Raw_Data(:,30:32); % helpmat locates the rows in which PMT data was read
        Data_Readings = Raw_Data((sum(helpstr, 2) == 3),1:28); % creates a vector only containing PMT data
        Time_of_Arrival = bin2dec(Data_Readings(:,1:28));        
end

...
switch PMT_Channel_Num
    case 1
        ...
        if (isempty(START_Dataset) && isempty(STOP2_Dataset))
            [PhotonArray, MaxNumOfEventsInLine] = CreateDataList(TotalEvents, 0, STOP1_Dataset, []);
            Num_of_Lines = 0;
    ...
    case 2
        ...
        if (isempty(START_Dataset) && isempty(STOP1_Dataset))
            [PhotonArray, MaxNumOfEventsInLine] = CreateDataList(TotalEvents, 0, STOP2_Dataset, []);
            Num_of_Lines = 0;
            StartOfFramesChannel = 0;
            MaxNumOfEventsInLine = 0;
        elseif isempty(START_Dataset)
    ...
    case 6
        ...
        if (isempty(STOP1_Dataset) && isempty(STOP2_Dataset))
            [PhotonArray, MaxNumOfEventsInLine] = CreateDataList(TotalEvents, 0, START_Dataset, []);
            Num_of_Lines = 0;
            StartOfFramesChannel = 0;
            MaxNumOfEventsInLine = 0;
        elseif isempty(STOP1_Dataset)
            ...
            %% Create the final cell array
            [PhotonArray, MaxNumOfEventsInLine] = CreateDataList(TotalEvents, Num_of_Lines, START_Dataset, STOP2_Dataset);
        elseif isempty(STOP2_Dataset)
            Num_of_Lines = numel(STOP1_Dataset.Time_of_Arrival) .* 2 - 1;
            StartOfFramesChannel = 0; % Since STOP2_Dataset contains line data, START contains frame data
            
            %% Create the final cell array
            [PhotonArray, MaxNumOfEventsInLine] = CreateDataList(TotalEvents, Num_of_Lines, START_Dataset, STOP1_Dataset);
